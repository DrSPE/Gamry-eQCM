...
also check if the crystal constant Cq in the script is shoosen correctly
...
direct = '/home/se/Dropbox/Masterarbeit/Projekt/data/raw/eQCM/C_beschichtet/'
crystNr = '#2'
date = '2016-05-19'
folder = 'QCM_RCVv8_10mV_all' 
filename = folder + '.DTA'
...
'''
imports the data from one cycle per loop, the interpolates the CV data since they have 
...
for i in range(len(cvname)):
    ...
    '''    
    np.savetxt(origin + 'eQCM_{0:02d}.txt'.format(i+1), np.c_[QCM, fs, fp, dfs, dfp,dm1, dm2, Q_new, U_new, I_new], delimiter=',', newline='\n', header='T/s , fs/Hz , fp/Hz, dfs, dfp, dm1/[ng/cm²], dm2/ng, Q/mC, U_Ref/V, I/µA')
    
    
    
   
    fig = plt.figure()
    fig.suptitle('Crystal ' + crystNr + ' coated with ' + carb + ', measured on ' + date + ': cycle {0:02d}'.format(i+1) )
    ...
    ax2.plot(U_new, dm2, 'g-')
    ax2.set_ylabel(r'$\Delta m / \mathrm{ng}$')
    ax2.set_xlabel(r'$U$' + U_label + r'$/ \mathrm{V}$')
    ...
    fig.savefig(origin + 'cycle_{0:02d}.pdf'.format(i+1), bbox_inches='tight')
    #plt.clf()
    plt.close()   
